...
// Todas las tasas de cambio (monedas + plataformas)
const exchangeRates: ExchangeRate[] = [
 // VES (Bolívares) como origen
 { from: 'VES', fromName: 'Bolívares', fromCode: 'VE', to: 'COP', toName: 'Pesos Colombianos', toCode: 'CO', rate: 42, symbol: '$' },
 { from: 'VES', fromName: 'Bolívares', fromCode: 'VE', to: 'PEN', toName: 'Soles', toCode: 'PE', rate: 0.0383, symbol: 'S/' },
 { from: 'VES', fromName: 'Bolívares', fromCode: 'VE', to: 'CLP', toName: 'Pesos Chilenos', toCode: 'CL', rate: 10.47, symbol: '$' },
 { from: 'VES', fromName: 'Bolívares', fromCode: 'VE', to: 'PAB', toName: 'Balboas', toCode: 'PA', rate: 0.015, symbol: 'B/.' },
 { from: 'VES', fromName: 'Bolívares', fromCode: 'VE', to: 'USDT', toName: 'Binance (USDT)', toCode: 'USDT', rate: 0.015, symbol: '$' },
 { from: 'VES', fromName: 'Bolívares', fromCode: 'VE', to: 'ZELLE', toName: 'Zelle', toCode: 'ZELLE', rate: 1, symbol: '$' },
 // COP (Pesos Colombianos) como origen
 { from: 'COP', fromName: 'Pesos Colombianos', fromCode: 'CO', to: 'VES', toName: 'Bolívares', toCode: 'VE', rate: 300, symbol: 'Bs.S' },
 { from: 'COP', fromName: 'Pesos Colombianos', fromCode: 'CO', to: 'PEN', toName: 'Soles', toCode: 'PE', rate: 8, symbol: 'S/' },
 { from: 'COP', fromName: 'Pesos Colombianos', fromCode: 'CO', to: 'CLP', toName: 'Pesos Chilenos', toCode: 'CL', rate: 7, symbol: '$' },
 { from: 'COP', fromName: 'Pesos Colombianos', fromCode: 'CO', to: 'PAB', toName: 'Balboas', toCode: 'PA', rate: 6, symbol: 'B/.' },
 { from: 'COP', fromName: 'Pesos Colombianos', fromCode: 'CO', to: 'USDT', toName: 'Binance (USDT)', toCode: 'USDT', rate: 51, symbol: '$' },
 // USD como origen
 { from: 'USD', fromName: 'Dólares', fromCode: 'US', to: 'VES', toName: 'Bolívares', toCode: 'VE', rate: 42.50, symbol: 'Bs.S' },
 { from: 'USD', fromName: 'Dólares', fromCode: 'US', to: 'COP', toName: 'Pesos Colombianos', toCode: 'CO', rate: 4150, symbol: '$' },
 { from: 'USD', fromName: 'Dólares', fromCode: 'US', to: 'PEN', toName: 'Soles', toCode: 'PE', rate: 3.75, symbol: 'S/' },
 { from: 'USD', fromName: 'Dólares', fromCode: 'US', to: 'CLP', toName: 'Pesos Chilenos', toCode: 'CL', rate: 950, symbol: '$' },
 { from: 'USD', fromName: 'Dólares', fromCode: 'US', to: 'PAB', toName: 'Balboas', toCode: 'PA', rate: 1, symbol: 'B/.' },
 // EUR como origen
 { from: 'EUR', fromName: 'Euros', fromCode: 'EU', to: 'VES', toName: 'Bolívares', toCode: 'VE', rate: 46.20, symbol: 'Bs.S' },
 { from: 'EUR', fromName: 'Euros', fromCode: 'EU', to: 'COP', toName: 'Pesos Colombianos', toCode: 'CO', rate: 4500, symbol: '$' },
 // PayPal y Zinli
 { from: 'PAYPAL', fromName: 'PayPal', fromCode: 'PAYPAL', to: 'VES', toName: 'Bolívares', toCode: 'VE', rate: 40.50, symbol: 'Bs.S' },
 { from: 'ZINLI', fromName: 'Zinli', fromCode: 'ZINLI', to: 'VES', toName: 'Bolívares', toCode: 'VE', rate: 41.00, symbol: 'Bs.S' },
];
...
export default function HomePage() {
 ...
 // Componente de bandera/icono por código - usa SVG externos para países
 const CountryFlag = ({ code, size = 48 }: { code: string; size?: number }) => {
  ...
  // Iconos de plataformas de pago (mantener como elementos inline)
  ...
  return <span className="flex-shrink-0">{platformIcons[code] || null}</span>;
 };
 ...
 return (
  <>
   ...
   <div className="sticky top-0 z-[60] bg-gradient-to-r from-[#05294F] to-[#07478F] py-2 overflow-hidden">
    <div className="animate-marquee whitespace-nowrap flex items-center gap-12">
     {[...exchangeRates, ...exchangeRates].map((rate, i) => (
      <span key={i} className="inline-flex items-center gap-2 text-white/90 text-sm">
       <CountryFlag code={rate.fromCode} size={20} />
       <span className="font-semibold">{rate.fromName}</span>
       <ArrowRight size={16} className="text-amber-400 flex-shrink-0" />
       <CountryFlag code={rate.toCode} size={20} />
       <span className="font-semibold">{rate.toName}</span>
       <span className="text-amber-400 font-bold">{rate.symbol} {formatNumber(rate.rate)}</span><span className="text-white/30 mx-4">|</span>
      </span>
     ))}
    </div>
   </div>
         ...
         <div className="mb-4">
          ...
          <div className="relative">
           ...
           >
            ...
            <option value="PAB">🇵🇦 Balboas (PAB)</option>
    ...
    <section id="tasas" className="py-20 bg-gradient-to-br from-slate-100 to-blue-100">
     <div className="container mx-auto px-4">
      ...
      <div className="flex flex-wrap justify-center gap-2 mb-8">
       {['USD', 'EUR', 'VES', 'COP', 'PAYPAL', 'ZINLI'].map((tab) => (
        <button
         key={tab}
      ...
      <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-xl overflow-hidden">
       ...
       {exchangeRates.filter(r => r.from === activeTab).map((rate, i) => {
        const key = `${rate.from}-${rate.to}`; const inputAmount = tableAmounts[key] ?? "1"; const calculatedAmount = (parseFloat(inputAmount) || 0) * rate.rate; return (
         <div key={i} className="px-6 py-4 grid grid-cols-4 items-center border-b border-gray-100 hover:bg-gray-50 transition-colors">
          <span className="flex items-center gap-2">
           <CountryFlag code={rate.fromCode} size={24} /><span className="text-xs font-medium text-gray-600">{rate.from}</span><ArrowRight size={14} className="text-gray-400" />
           <CountryFlag code={rate.toCode} size={24} /><span className="text-xs font-medium text-gray-600">{rate.to}</span></span>
          <div className="flex items-center justify-center"><input type="number" value={inputAmount} onChange={(e) => setTableAmounts(prev => ({ ...prev, [key]: e.target.value }))} className="w-20 text-center border border-gray-200 rounded-lg px-2 py-1 text-sm font-medium focus:ring-2 focus:ring-[#05294F] focus:border-transparent" min="0" /><span className="ml-1 text-xs text-gray-500">{rate.from}</span></div>
          <span className="text-center font-bold text-lg text-green-600">{rate.symbol} {formatNumber(calculatedAmount)}</span>
       ...
       {exchangeRates.filter(r => r.from === activeTab).length === 0 && (

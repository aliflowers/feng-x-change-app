...
interface ExtendedUserBankAccount extends UserBankAccount {
  banks_platforms: {
    name: string;
    currency_id: number;
    // We could join currency to get code, or just infer from context/logic
    currencies: {
      code: string;
      symbol: string;
    }
            ...
            <div key={acc.id} className="group relative bg-white border border-gray-200 rounded-2xl p-6 hover:shadow-xl transition-all duration-300 hover:border-blue-300">
              <div className="flex justify-between items-start mb-5">
                <div className="flex items-center gap-4">
                  {(() => {
                    const logoUrl = getBankLogoUrl(acc.banks_platforms?.name || '');
                    return logoUrl ? (
                      <img
                        src={logoUrl}
                        alt={acc.banks_platforms?.name}
                  ...
                  <div className={`w-14 h-14 rounded-xl flex items-center justify-center ${getBankLogoUrl(acc.banks_platforms?.name || '') ? 'hidden' : ''
                    } ${acc.banks_platforms?.currencies?.code === 'VES' ? 'bg-yellow-100 text-yellow-700' :
                      acc.banks_platforms?.currencies?.code === 'USD' ? 'bg-green-100 text-green-700' :
                        'bg-blue-100 text-blue-700'
                    }`}>
                    <Building2 size={28} />
                  </div>
                  <div>
                    <h4 className="text-lg font-bold text-gray-900 leading-tight">{acc.banks_platforms?.name}</h4>
                    <span className="text-sm font-semibold text-gray-500 bg-gray-100 px-3 py-1 rounded-full">
                      {acc.banks_platforms?.currencies?.code}
                    </span>
                  </div>
                </div>
                  ...
                  <tr key={acc.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-semibold text-gray-900">{acc.banks_platforms?.name}</p>
